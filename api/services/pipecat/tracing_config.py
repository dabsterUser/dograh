...
class _OrgAttributeSpanProcessor(SpanProcessor):
    ...
    def on_start(self, span, parent_context=None):
        ...
        if org_id:
            span.set_attribute("callagent.org_id", str(org_id))
...
class _OrgRoutingExporter(SpanExporter):
    ...
    Spans with a ``callagent.org_id`` attribute whose org has registered
    ...
    def export(self, spans):
        ...
        for span in spans:
            ...
            org_id = span.attributes.get("callagent.org_id") if span.attributes else None
...
def ensure_tracing() -> bool:
    ...
    setup_tracing(service_name="callagent-pipeline", exporter=_org_routing_exporter)
