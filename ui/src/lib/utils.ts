...
export function setStackRefreshCookie(refreshToken: string) {
  ...
  const isCallAgentDomain = window.location.hostname.endsWith('.callagent.ai');
  const cookieDomainPart = isCallAgentDomain ? '; domain=.callagent.ai' : '';
...
export async function impersonateAsSuperadmin(params: {
  ...
  // (served from the target sub-domain, e.g. app.callagent.ai). The route will set the
  ...
  // superadmin.dograh.com we want to switch to app.callagent.ai. For any other domain
